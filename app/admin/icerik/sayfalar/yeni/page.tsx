...
export default function NewPagePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        showToast("success", "Sayfa basariyla olusturuldu");
        setTimeout(() => router.push("/admin/icerik/sayfalar"), 500);
      } else {
        showToast("error", json.message || "Sayfa olusturulurken hata olustu");
      }
    } catch {
      showToast("error", "Sayfa olusturulurken hata olustu");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold font-heading text-foreground">Yeni Sayfa</h1>
          <p className="text-muted text-sm">Yeni bir CMS sayfasi olusturun</p>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {saving ? "Kaydediliyor..." : "Sayfayi Olustur"}
          </button>
          <button
            type="button"
            onClick={() => router.push("/admin/icerik/sayfalar")}
