...
export default function ActivitiesPage() {
  ...
  const fetchPosts = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/posts?type=ACTIVITY");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-heading text-foreground">Faaliyetler</h1>
          <p className="text-muted text-sm">Faaliyet içeriklerini yönetin</p>
        </div>
        <Link href="/admin/icerik/faaliyetler/yeni" className="btn-primary px-5 py-2.5 text-sm inline-flex items-center gap-2">
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-muted">Henüz faaliyet eklenmemiş</div>
        ) : (
            ...
            {posts.map((post) => (
              <div key={post.id} className="px-6 py-4 flex items-center justify-between hover:bg-background-alt/50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link href={`/admin/icerik/faaliyetler/${post.id}`} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-primary/10 text-primary hover:bg-primary/20">Düzenle</Link>
